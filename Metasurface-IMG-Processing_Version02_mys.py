...
# =============================================================================
scaling = 500 # make this a relatively big number. even if you need to change units 
unit = 'nm'
#threshold=250
threshold=150 # change its value to adjust performance
path = 'C:/Users/shala/Desktop/_python codes/metasurface-sem-img-main/docs/assets/images/'
img_name = 'No0002_'
img_fullname = path + img_name+'.tif'
num_sizes = 10
...
'''
perpixel = 2.66 #inputed from ImageJ 
...
rad_list = [float(rad_list[i]) for i in range(len(rad_list))]
print(np.mean(rad_list))
# data analysis 
...
filename = img_name + '.csv'
save_to_csv(rad_list, filename)
...
plt.imshow(img)
plt.savefig(img_name+'diameter_roundness_map'+'.png',dpi=300)

plt.imshow(imgcopy)
plt.savefig(img_name+'area_colormap'+'.png',dpi=300)

plt.imshow(edges)
plt.savefig(img_name+'canny_edge'+'.png',dpi=300)
